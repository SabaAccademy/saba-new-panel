...
export interface FormAdapterProviderProps {
  ...
  /** Optional schema – passed by Smart components for field-default initialisation in legacy adapters */
  schema?: FormSchema;
}
...
// ─── SmartWizardForm Props ─────────────────────────────────────────────────────
export interface SmartWizardFormProps {
  steps: WizardStep[];
  adapter: FormAdapter;
  ui: UIAdapter;
  onSubmit: (values: Record<string, unknown>) => void | Promise<void>;
