...
export function maskCurrency(value: string): string {
  // toLocaleString("fa-IR") outputs Persian digits (e.g. ۱٬۵۰۰٬۰۰۰).
  // On the next keystroke the stored Persian value is fed back, so we must
  // normalise Persian/Arabic-Indic digits → ASCII before stripping separators.
  const ascii = String(value).replace(/[\u06F0-\u06F9\u0660-\u0669]/g, (c) =>
    String(c.charCodeAt(0) - (c <= "\u06F9" ? 0x06f0 : 0x0660)),
  );
