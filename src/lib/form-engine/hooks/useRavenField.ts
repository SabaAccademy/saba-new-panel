...
export function useRavenField({
  ...
  // When a mask is active the stored value is already the masked display string;
  // applying the parser here would convert it to a raw number and destroy formatting.
  // Parser is only used at submission time (handled by the adapter), not for display.
