...
function analyseField(
  ...
  if (!nowEmpty && typeof current === "string" && current.trim().length === 0) {
    hints.push({
      kind: "WHITESPACE",
      severity: "warning",
  ...
  const numericName =
    /price|cost|amount|qty|quantity|stock|count|rate|score|weight|total|tax/i.test(
      name,
    );
  if (
    numericName &&
...
const SEVERITY_STYLES: Record<HintSeverity, { pill: string; dot: string }> = {
  success: {
    pill: "bg-emerald-500/15 text-emerald-400 border-emerald-800/40",
    dot: "bg-emerald-500",
  },
  info: {
    pill: "bg-sky-500/15 text-sky-400 border-sky-800/40",
    dot: "bg-sky-400",
  },
  warning: {
    pill: "bg-amber-500/15 text-amber-400 border-amber-800/40",
    dot: "bg-amber-400",
  },
  danger: {
    pill: "bg-rose-500/15 text-rose-400 border-rose-800/40",
    dot: "bg-rose-500",
  },
  neutral: {
    pill: "bg-zinc-700/50 text-zinc-400 border-zinc-700",
    dot: "bg-zinc-500",
  },
};
      ...
      {expanded && (
        <div className="border-t border-zinc-800 px-3 py-2 flex flex-col gap-1.5 bg-zinc-950/60">
          ...
          <div className="mt-1 pt-1.5 border-t border-zinc-800/60 grid grid-cols-2 gap-2">
            <div>
              <p className="text-[9px] text-zinc-600 uppercase tracking-wide mb-0.5">
                Initial
              </p>
  ...
  return (
    <div className="flex flex-col">
      <p className="text-[10px] text-zinc-600 mb-2 font-mono">
        {dirtyEntries.length} field{dirtyEntries.length !== 1 ? "s" : ""}{" "}
        modified · click a row to expand hints
      </p>
